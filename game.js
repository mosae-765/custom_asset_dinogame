...
class BitmapCache {
  ...
  _prescale(src, targetW, targetH) {
    ...
    // 50%씩 단계적으로 축소해 고품질 다운스케일
    let cur = src, curW = srcW, curH = srcH;
    while (curW > targetW * 2 || curH > targetH * 2) {
      const stepW = Math.max(targetW, Math.floor(curW / 2));
      const stepH = Math.max(targetH, Math.floor(curH / 2));
      const oc = new OffscreenCanvas(stepW, stepH);
      const ctx = oc.getContext('2d');
      ctx.imageSmoothingEnabled = true;
      ctx.imageSmoothingQuality = 'high';
      ctx.drawImage(cur, 0, 0, stepW, stepH);
      cur = oc; curW = stepW; curH = stepH;
    }

    if (curW === targetW && curH === targetH) return cur;
